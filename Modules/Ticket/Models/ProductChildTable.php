...
class ProductChildTable extends Model
{
    ...
    /**
     * lấy danh sách sản phẩm có số lượng lớn hơn 0
     * @param $filter
     * @return mixed
     */
    public function getProductsUsing($filter)
    {
        $imageDefault = 'http://' . request()->getHttpHost() . '/static/images/product.png';
        ...
        $ds = $this
            ...
            ->where("product_categories.is_actived", self::IS_ACTIVE)
            ->where("product_categories.is_deleted", self::NOT_DELETE)
            ->where(DB::raw("(Select SUM(product_inventorys.quantity) FROM product_inventorys where product_inventorys.product_id = {$this->table}.product_child_id)"),'>',0);
