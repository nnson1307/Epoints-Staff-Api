...
class UploadFileToAzureStorage implements UploadFileToAzureManager
{
    ...
    {
        $this->blobClient = BlobRestProxy::createBlobService(env('CONNECTION_STRING'));
    }
    ...
    public function doUpload($file)
    {
        ...
        if ($handle) {
            try {
                ...
                $this->blobClient->createBlockBlob($containerName, $fileName, $handle, $options);
    ...
    public function deleteBlobImage($file_path)
    {
        try {
            ...
            $containerName = env('CONTAINER_NAME');
            if (!empty($fileName) && !empty($containerName)) {
